...
class MessageSave:
    ...
    def save(self):
        ...
        while True:
            ...
            if re.fullmatch('^From:.*', dataLine) or re.fullmatch('^To:.*', dataLine) or re.fullmatch('^Subject:.*', dataLine) or re.fullmatch('^Date:.*', dataLine):      
                Body += dataLine + self.CRLF
            # Break the while loop if all the header lines have been processed
            elif (not isHeader) or (dataLine==''):                                
                break
            # Check all the tags in Content-Type (may be multiple line)
                ...
                elif not re.fullmatch('Content-Type:.*', dataLine):          
                    contentType = False
                ...
                for parameter in attribute:
                    ...
                    if re.fullmatch('multipart.*', parameter):               
                        multipart = True
        ...
        while True:
            ...
            if mime and (dataLine == '' or dataLine == ('--'+boundary)):       
                break
            # if the Email is non-MIME, Single Part or without header, save the body
            encodedBody += dataLine +self.CRLF                                 

        # Do it only if the Email is non-MIME, Single Part or without header            print('mime2:  '+str(mime))
        if (not isHeader) or (not mime) or (not multipart):                       
        ...
        # This block is only for Multipart Message
            ...
            while True:
                ...
                if isHeader:
                    ...
                    elif re.fullmatch('Content-Transfer-Encoding:\s*base64\s*', dataLine):         
                        base64Encoded = True                                 
